...
class GMPicker: UIView {
    ...
    struct Config {
        ...
        fileprivate let contentHeight: CGFloat = 190
    ...
    func setupGender(){
        ...
        self.Array = ["Women", "Men"]
        gmpicker.reloadAllComponents()
    }
    
    func setupAllGender(){
        
        self.Array = ["Women", "Men", "Men & Women"]
        gmpicker.reloadAllComponents()
    }
    ...
    fileprivate func setup(_ parentVC: UIViewController) {
        ...
        confirmButton.titleLabel?.font = UIFont(name: Font_Dosis_Regular, size: config.buttonFontSize)
        ...
        cancelButton.titleLabel?.font = UIFont(name: Font_Dosis_Regular, size: config.buttonFontSize)
...
//        placementAnswer = Array[Array.endIndex - 1]
    ...
    func show(inVC parentVC: UIViewController, completion: (() -> ())? = nil) {
        ...
        UIView.animate(
            withDuration: config.animationDuration, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 5, options: .curveEaseOut, animations: {
