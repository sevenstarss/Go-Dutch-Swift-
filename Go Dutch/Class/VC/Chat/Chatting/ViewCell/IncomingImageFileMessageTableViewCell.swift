...
class IncomingImageFileMessageTableViewCell: UITableViewCell {
    ...
    func setModel(aMessage: SBDFileMessage) {
        ...
        if self.message.thumbnails != nil && (self.message.thumbnails?.count)! > 0 {
            self.fileImageView.af_setImage(withURL: URL(string: (self.message.thumbnails?[0].url)!)!, placeholderImage: Utils.imageFromColor(color: ChatConstants.incomingFileImagePlaceholderColor()))
        }
        else {
            self.fileImageView.af_setImage(withURL: URL(string: self.message.url)!, placeholderImage: Utils.imageFromColor(color: ChatConstants.incomingFileImagePlaceholderColor()))
        }
