...
class OutgoingUserMessageTableViewCell: UITableViewCell {
    ...
    func setModel(aMessage: SBDUserMessage) {
        ...
        let dateSeperator = seperatorDateFormatter.string(from: messageCreatedDate as Date)
        self.dateSeperatorLabel.text = (dateSeperator.contains("1970")) ?  seperatorDateFormatter.string(from: NSDate() as Date) : dateSeperator
    ...
    func buildMessage() -> NSAttributedString {
        let messageAttribute = [
            NSFontAttributeName: ChatConstants.messageFont()
        ]
